...
export class World {
  ...
  constructor(options: WorldOptions) {
    ...
    this.params = options.params ? { ...options.params } : { ...DEFAULT_PARAMS };
    ...
    // Cell size only affects query efficiency, not correctness (forEachNear
    // always takes the live radius as a parameter) — so it's fine to size
    // these grids once from the initial params even though attackRadius/
    // mateSeekRadius/tradeRadius can change live afterward.
    this.creatureGrid = new SpatialGrid(this.worldWidth, this.worldHeight, this.params.attackRadius);
    this.matingGrid = new SpatialGrid(this.worldWidth, this.worldHeight, this.params.mateSeekRadius);
    this.tradeGrid = new SpatialGrid(this.worldWidth, this.worldHeight, this.params.tradeRadius);
  ...
  step(dt: number): void {
    ...
    for (let i = this.creatureCount - 1; i >= 0; i--) {
      ...
      const metabolism =
        (this.params.metabolismBase + this.params.metabolismSpeedFactor * this.creatureSpeedGene[i]) *
      ...
      if (nearestFood >= 0 && nearestFood < this.foodCount) {
        ...
        if (dx * dx + dy * dy <= EAT_RADIUS * EAT_RADIUS) {
          this.creatureEnergy[i] = Math.min(ENERGY_CAP, this.creatureEnergy[i] + this.params.foodEnergy);
  ...
  private resolvePredation(): void {
    ...
    for (let i = this.creatureCount - 1; i >= 0; i--) {
      if (this.creatureEnergy[i] >= this.params.predationHungerThreshold) continue;

      const cx = this.creaturePosX[i];
      const cy = this.creaturePosY[i];
      const kinTolerance = this.creatureKinTolerance[i];
      const attackRadius = this.params.attackRadius;

      let victim = -1;
      let victimDistSq = attackRadius * attackRadius;
      this.creatureGrid.forEachNear(cx, cy, attackRadius, (other) => {
      ...
      if (victim >= 0 && victim < this.creatureCount) {
        this.creatureEnergy[i] = Math.min(
          ENERGY_CAP,
          this.creatureEnergy[i] + this.params.predationEfficiency * this.creatureEnergy[victim],
        );
  ...
  private resolveTrading(): void {
    ...
    for (let i = 0; i < snapshot; i++) {
      ...
      const tradeRadius = this.params.tradeRadius;

      let bestPartner = -1;
      let bestScore = Infinity;
      this.tradeGrid.forEachNear(cx, cy, tradeRadius, (other) => {
        ...
        const dy = this.creaturePosY[other] - cy;
        const distSq = dx * dx + dy * dy;
        if (distSq > tradeRadius * tradeRadius) return;

        const spatialScore = distSq / (tradeRadius * tradeRadius);
  ...
  private executeTrade(a: number, b: number): void {
    ...
    const tradeChunk = this.params.tradeChunk;
    const aGivesA = Math.min(surplusAofA, deficitAofB, tradeChunk);
    const bGivesB = Math.min(surplusBofB, deficitBofA, tradeChunk);
    // Case 2 (mirror): a gives B, b gives A.
    const aGivesB = Math.min(surplusBofA, deficitBofB, tradeChunk);
    const bGivesA = Math.min(surplusAofB, deficitAofA, tradeChunk);
  ...
  private isReadyToMate(i: number): boolean {
    return (
      this.creatureEnergy[i] >= this.params.reproduceThreshold &&
  ...
  private resolveMating(): void {
    ...
    for (let i = 0; i < readyCountSnapshot; i++) {
      ...
      const cx = this.creaturePosX[i];
      const cy = this.creaturePosY[i];
      const mateSeekRadius = this.params.mateSeekRadius;

      let mate = -1;
      let mateDistSq = mateSeekRadius * mateSeekRadius;
      this.matingGrid.forEachNear(cx, cy, mateSeekRadius, (other) => {
  ...
  climateMultiplier(): number {
    const period = this.params.climatePeriodTicks;
    const phase = this.tick % period;
    return phase < period / 2 ? this.params.climateScarcityMultiplier : this.params.climateAbundanceMultiplier;
  }

  private spawnFoodOverTime(dt: number): void {
    const emptySlots = this.foodCapacity - this.foodCount;
    if (emptySlots <= 0) return;
    const expected = this.params.foodRegrowthProbability * this.climateMultiplier() * emptySlots * dt;
  ...
  private spawnFoodAOverTime(dt: number): void {
    ...
    if (emptySlots <= 0) return;
    const expected = this.params.foodRegrowthProbability * emptySlots * dt;
    let spawns = Math.floor(expected);
    if (this.rng.next() < expected - spawns) spawns++;
  ...
  private reproduceSexual(parentA: number, parentB: number): void {
    ...
    this.creatureReserveGene[childIndex] = this.recombine(
      ...
      this.params.genomeMutationStdDev,
      RESERVE_GENE_MIN,
      RESERVE_GENE_MAX,
    );
    ...
    this.creatureKinTolerance[childIndex] = this.recombine(
      ...
      this.params.kinToleranceMutationStdDev,
      0,
      MAX_GENOME_DISTANCE,
    );
    ...
    this.creatureKinTradeBias[childIndex] = this.recombine(
      ...
      this.params.kinTradeBiasMutationStdDev,
      0,
      1,
    );
  ...
  // Merges a partial update into the live params — lets the UI push a
  // single changed slider without resending the whole set.
  ...
  loadSnapshot(snapshot: WorldSnapshot): void {
    if (snapshot.version !== 5) {
      throw new Error(`Versión de guardado incompatible: se esperaba 5, el archivo tiene ${snapshot.version}`);
    }
